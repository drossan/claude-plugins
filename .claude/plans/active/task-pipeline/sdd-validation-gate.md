...
- [ ] `task-pipeline-sdd-validation-gate-02` (P2) — **Bash helper opcional** `sdd-lint.sh` (subconjunto determinista, NO bloqueante, best-effort) + **fixtures aseverados** known-good/known-bad + doc "cómo cablearlo a tu CI"  · depends_on: 01
- [ ] `task-pipeline-sdd-validation-gate-03` (P1) — Gate en el cierre: línea de DoD gated `features.sdd` en `task.md` + 2×lifecycle + wiring "Cerrar una tarea" (`mutation → sdd-lint → fact-checker`) + nota `plan-task` + reconocimiento en `/doctor` (ausencia ≠ drift)  · depends_on: 01
