...
> **Modelo por fase (opcional)**: puedes fijar el modelo de las 3 fases siempre ruteables
> (`design-review`/`scenario-coverage`/`fact-checker`) + `sdd-lint` (condicional, solo con `features.sdd`
> on) en la sección `models:` de `.claude/task-pipeline.yml`. Ver [Routing de modelo por fase](task-pipeline/README.md#routing-de-modelo-por-fase-models) en el README del plugin.
